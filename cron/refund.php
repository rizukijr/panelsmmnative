...
if (mysqli_num_rows($check_order) == 0) {
	...
	while($data_order = mysqli_fetch_assoc($check_order)) {
		...
		$u_remains = $data_order['remains'];
			...
			$update_order = mysqli_query($db, "INSERT INTO balance_history (id, username, action, quantity, msg, date, time) VALUES ('', '$buyer', 'Add Balance', '$refund', 'Pengembalian Dana Dari Pemesanan Pada Fitur Sosial Media Akibat Status Pesanan Error/Partial Dengan Order ID $o_oid', '$date', '$time')");
