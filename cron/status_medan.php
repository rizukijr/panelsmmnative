...
$check_order = mysqli_query($db, "SELECT * FROM orders WHERE status IN ('','Pending','Processing') AND provider = 'MP-SOSMED'");
...
if (mysqli_num_rows($check_order) == 0) {
  ...
  if ($o_provider == "MANUAL") {
    ...
    $check_provider = mysqli_query($db, "SELECT * FROM provider WHERE code = 'MP-SOSMED'");
