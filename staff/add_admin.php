...
if (isset($_SESSION['user'])) {
	...
	if (mysqli_num_rows($check_user) == 0) {
		...
		header("Location: ".$cfg_baseurl."logout.php");
	} else if ($data_user['level'] == "Reseller" OR $data_user['level'] == "Admin" OR $data_user['level'] == "Member") {
		...
		if (isset($_POST['add'])) {
			...
			if (empty($post_username) || empty($post_password)) {
				...
				if ($update_user == TRUE) {
                $insert_user = mysqli_query($db, "INSERT INTO balance_history (username, action, quantity, msg, date, time) VALUES ('$sess_username', 'Cut Balance', '$post_price' , 'Saldo dipotong untuk Penambahan user : $post_username dengan level  Admin', '$date', 'time')");				        	
				$insert_user = mysqli_query($db, "INSERT INTO users (username, password, balance, level, registered, status, api_key, email, uplink) VALUES ('$post_username', '$post_password', '$post_balance', 'Admin', '$date', 'Active', '$post_api', '$post_email', '$sess_username')");
				if ($insert_user == TRUE) {
                            ...
                            <div class="col-md-2"></div><div class="col-md-8">
                                <div class="panel panel-color panel-info">
                                    <div class="panel-heading">
                                        <h3 class="panel-title"><i class="mdi mdi-account-multiple-outline"></i> Tambah Admin</h3>
                                    </div>
